...
void encode(std::ostream &stream, uint64_t value)
{
    unsigned char bytes[] = {
        static_cast<unsigned char>(value & 0xFF),
        static_cast<unsigned char>(value >> 8)
...
bool SoundFileWriterWav::open(const std::string &filename, unsigned int sampleRate, unsigned int channelCount)
{
    ...
    if (!m_file) {
        std::cerr << "Failed to open WAV sound file \"" << filename << "\" for writing" << std::endl;
        return false;
    }

    // Write the header
    if (!writeHeader(sampleRate, channelCount)) {
        std::cerr << "Failed to write header of WAV sound file \"" << filename << "\"" << std::endl;
        return false;
    }
...
////////////////////////////////////////////////////////////
void SoundFileWriterWav::write(const uint32_t *samples, uint64_t count)
{
    assert(m_file.good());
...
void SoundFileWriterWav::close()
{
    ...
    if (m_file.is_open()) {
        ...
        uint32_t dataChunkSize = static_cast<uint32_t>(m_sampleCount * m_channelCount * 2);
