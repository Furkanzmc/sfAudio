...
SoundRecorder::~SoundRecorder()
{
    ...
    assert(!m_isCapturing
           && "You must call stop() in the destructor of your derived class, so that the recording thread finishes before your object is destroyed.");
}
...
bool SoundRecorder::start(unsigned int sampleRate)
{
    ...
    if (!isAvailable()) {
        std::cerr << "Failed to start capture: your system cannot capture audio data(call SoundRecorder::isAvailable to check it)" << std::endl;
        return false;
    }
    ...
    if (!captureDevice) {
        std::cerr << "Failed to open the audio capture device with the name: " << m_deviceName << std::endl;
        return false;
    }
...
{
    if (m_isCapturing) {
        std::cerr << "It's not possible to change the channels while recording." << std::endl;
        return;
    }

    if (channelCount < 1 || channelCount > 2) {
        std::cerr << "Unsupported channel count: " << channelCount << " Currently only mono (1) and stereo (2) recording is supported." << std::endl;
        return;
    }
...
void SoundRecorder::record()
{
    while (m_isCapturing) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(m_processingInterval.asMilliseconds()));
    }
