...
////////////////////////////////////////////////////////////
void SoundFileReaderOgg::seek(uint64_t sampleOffset)
{
...
////////////////////////////////////////////////////////////
uint64_t SoundFileReaderOgg::read(int16_t *samples, uint64_t maxCount)
{
    assert(m_vorbis.datasource);
    ...
    while (count < maxCount) {
        int bytesToRead = static_cast<int>(maxCount - count) * sizeof(int16_t);
        long bytesRead = ov_read(&m_vorbis, reinterpret_cast<char *>(samples), bytesToRead, 0, 2, 1, NULL);
        if (bytesRead > 0) {
            long samplesRead = bytesRead / sizeof(int16_t);
            count += samplesRead;
            samples += samplesRead;
        }
        else {
