...
////////////////////////////////////////////////////////////
bool SoundBuffer::loadFromSamples(const int16_t *samples, uint64_t sampleCount, unsigned int channelCount, unsigned int sampleRate)
{
...
bool SoundBuffer::initialize(InputSoundFile &file)
{
    ...
    uint64_t       sampleCount  = file.getSampleCount();
...
bool SoundBuffer::update(unsigned int channelCount, unsigned int sampleRate)
{
    ...
    if (format == 0) {
        std::cerr << "Failed to load sound buffer(unsupported number of channels: " << channelCount << ")" << std::endl;
        return false;
    }
