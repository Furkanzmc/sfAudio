...
FLAC__StreamDecoderReadStatus streamRead(const FLAC__StreamDecoder *, FLAC__byte buffer[], std::size_t *bytes, void *clientData)
{
    ...
    uint64_t count = data->stream->read(buffer, *bytes);
    if (count > 0) {
...
bool SoundFileReaderFlac::open(InputStream &stream, Info &info)
{
    ...
    if (!m_decoder) {
        std::cerr << "Failed to open FLAC file (failed to allocate the decoder)" << std::endl;
        return false;
    }

    // Initialize the decoder with our callbacks
    m_clientData.stream = &stream;
    FLAC__stream_decoder_init_stream(m_decoder, &streamRead, &streamSeek, &streamTell, &streamLength, &streamEof, &streamWrite, &streamMetadata, &streamError,
                                     &m_clientData);

    // Read the header
    if (!FLAC__stream_decoder_process_until_end_of_metadata(m_decoder)) {
        close();
        std::cerr << "Failed to open FLAC file (failed to read metadata)" << std::endl;
        return false;
    }
...
////////////////////////////////////////////////////////////
void SoundFileReaderFlac::seek(uint64_t sampleOffset)
{
    assert(m_decoder);
...
////////////////////////////////////////////////////////////
uint64_t SoundFileReaderFlac::read(int16_t *samples, uint64_t maxCount)
{
    assert(m_decoder);
    ...
    if (left > 0) {
        if (left > maxCount) {
            ...
            std::vector<int16_t> leftovers(m_clientData.leftovers.begin() + maxCount, m_clientData.leftovers.end());
